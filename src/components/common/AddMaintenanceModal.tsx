    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-md bg-[#0B1226] border-2 border-[#FB923C]/40 rounded-[28px] p-4 sm:p-5 text-[#F4F8FF] shadow-[0_20px_60px_rgba(0,0,0,0.98)] space-y-4 max-h-[92vh] overflow-y-auto [&::-webkit-scrollbar]:hidden">
        ...
        <form onSubmit={handleSubmitForm} className="space-y-4">
          ...
          {/* 6. Next Service Due Date with Shortcuts */}
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-slate-300">Next Service Due</label>
              <div className="flex items-center gap-1">
                {[
                  ...
                  <button
                    key={item.label}
                    type="button"
                    onClick={() => handleQuickDue(item.months)}
                    className="text-[9px] px-1.5 py-0.5 rounded bg-[#0D152D] text-[#FB923C] border border-slate-700 hover:border-[#FB923C] transition-colors"
                  >
                    {item.label}
                  </button>
                ))}
            ...
            <div className="flex items-center bg-[#050811] border border-slate-700/80 rounded-xl px-3 py-2 text-white">
              <Calendar className="w-3.5 h-3.5 text-[#FB923C] mr-2 shrink-0" />
              <input
                type="date"
                value={nextDue}
