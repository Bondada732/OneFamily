      ...
      <div className="w-full max-w-md bg-[#061F5C] border-2 border-[#168BFF]/40 rounded-[28px] p-4 sm:p-5 text-[#F4F8FF] shadow-[0_20px_60px_rgba(3,25,74,0.98)] space-y-4 max-h-[92vh] overflow-y-auto [&::-webkit-scrollbar]:hidden">
        ...
        <div className="flex items-center justify-between pb-1">
          <div className="flex items-center gap-2">
            <div className="p-2 rounded-xl bg-[#0869E8]/20 text-[#7EDCFF] border border-[#168BFF]/40">
              <Wallet className="w-5 h-5 stroke-[2.2]" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-white tracking-tight">Add Family Income</h3>
              <p className="text-[10px] text-[#B9D8FF]">Directly deposits & credits to Family Wealth balance</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmitForm} className="space-y-4">
          ...
          {/* 5. Received Date with quick shortcuts */}
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-[#B9D8FF]">Received Date</label>
              <div className="flex items-center gap-1.5">
                {[
                  { label: 'Today', mode: 'today' as const },
                  { label: 'Yesterday', mode: 'yesterday' as const },
                  { label: '1st of Month', mode: 'first' as const },
                ].map((item) => (
                  <button
                    key={item.label}
                    type="button"
                    onClick={() => handleQuickDate(item.mode)}
                    className="text-[10px] font-bold px-2 py-0.5 rounded bg-[#073B9E]/60 text-[#16C7F2] border border-[#168BFF]/30 hover:bg-[#168BFF]/30 transition-colors"
                  >
                    {item.label}
                  </button>
                ))}
