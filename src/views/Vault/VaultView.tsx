  ...
  if (!canViewDocs) {
    return (
      <div className="p-6 text-center space-y-4 my-auto">
        <div className={`w-16 h-16 rounded-3xl border mx-auto flex items-center justify-center ${
          isLight ? 'bg-rose-100 border-rose-300 text-rose-600' : 'bg-rose-500/20 border-rose-500/30 text-rose-400'
        }`}>
          <ShieldAlert className="w-8 h-8" />
        </div>
        <h2 className={`text-lg font-bold ${isLight ? 'text-[#1F1F1F]' : 'text-white'}`}>Document Vault Restricted</h2>
        <p className={`text-xs max-w-xs mx-auto leading-relaxed ${isLight ? 'text-[#634B3F]' : 'text-slate-400'}`}>
    ...
    <div className={`p-4 space-y-5 animate-fade-in pb-12 ${isLight ? 'text-[#1F1F1F]' : 'text-slate-100'}`}>
        ...
        {canUploadDocs && (
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs flex items-center gap-1.5 font-bold transition-all cursor-pointer ${
              isLight
                ? 'bg-gradient-to-r from-[#FF8A3D] via-[#E05318] to-[#C2410C] hover:from-[#E05318] hover:to-[#B83808] text-white border-t border-white/30 border-b-[2.5px] border-b-[#9A3412] shadow-md shadow-orange-500/20 active:translate-y-0.5'
                : 'bg-gradient-to-r from-[#168BFF] to-[#0A56C2] hover:from-[#16C7F2] hover:to-[#168BFF] text-white shadow-lg shadow-[#168BFF]/25 active:scale-95'
            }`}
          >
      ...
      {expiringSoonDocs.length > 0 && (
        <div className={`p-3.5 rounded-2xl flex items-center gap-3 ${
          ...
          <AlertTriangle className={`w-5 h-5 shrink-0 ${isLight ? 'text-amber-600' : 'text-amber-400'}`} />
          <div className="text-xs">
            <strong className={isLight ? 'text-amber-900' : 'text-amber-300'}>{expiringSoonDocs.length} Documents Expiring Soon:</strong>{' '}
      ...
      <div className="space-y-2">
        <div className={`flex items-center gap-2 px-3 py-2 rounded-2xl border transition-colors ${
          isLight
            ? 'bg-[#F3E3D3]/80 border-[#DEC8B2] focus-within:border-[#E05318]'
            : 'bg-slate-800/90 border-slate-700/80'
        }`}>
          <Search className={`w-4 h-4 ${isLight ? 'text-[#8A6D5E]' : 'text-slate-400'}`} />
          <input
            type="text"
            ...
            className={`w-full bg-transparent text-xs outline-none ${
              isLight ? 'text-[#1F1F1F] placeholder-[#8A6D5E]' : 'text-white placeholder-slate-400'
            }`}
          />
        </div>
      ...
      {previewDoc && createPortal(
        <div className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 ${isLight ? 'bg-black/50' : 'bg-black/85'} backdrop-blur-md animate-fade-in select-none`}>
          <div className={`w-full max-w-md rounded-3xl p-5 shadow-2xl space-y-4 ${
            isLight
              ? 'bg-[#FFF8F1] border-2 border-[#DEC8B2] text-[#1F1F1F]'
              : 'bg-[#07132B] border border-[#168BFF]/30 text-slate-100'
          }`}>
            <div className={`flex items-center justify-between pb-2 border-b ${isLight ? 'border-[#DEC8B2]' : 'border-slate-800'}`}>
              <div className="flex items-center gap-2">
                <FolderLock className={`w-5 h-5 ${isLight ? 'text-[#E05318]' : 'text-amber-400'}`} />
                <h3 className={`text-base font-bold truncate ${isLight ? 'text-[#1F1F1F]' : 'text-white'}`}>{previewDoc.title}</h3>
              </div>
              <button
                onClick={() => setPreviewDoc(null)}
                className={`transition-colors cursor-pointer ${isLight ? 'text-[#634B3F] hover:text-[#1F1F1F]' : 'text-slate-400 hover:text-white'}`}
              >
                ✕
              </button>
            </div>
            ...
            {previewDoc.file_url && (previewDoc.file_url.startsWith('data:image') || previewDoc.file_type === 'IMAGE') ? (
              <div className={`max-h-56 rounded-2xl overflow-hidden relative border flex items-center justify-center ${
                isLight ? 'border-[#DEC8B2] bg-[#F8EFE6]' : 'border-slate-700 bg-slate-950'
              }`}>
                <img src={previewDoc.file_url} alt={previewDoc.title} className="max-h-56 w-auto object-contain rounded-2xl" />
              </div>
            ) : (
              <div className={`h-44 rounded-2xl overflow-hidden relative border flex flex-col items-center justify-center p-4 text-center ${
                isLight ? 'border-[#DEC8B2] bg-[#F8EFE6]' : 'border-slate-700 bg-slate-950'
              }`}>
                <FileText className={`w-12 h-12 mb-2 ${isLight ? 'text-[#E05318]' : 'text-[#16C7F2]'}`} />
                <div className={`text-xs font-bold truncate max-w-xs ${isLight ? 'text-[#1F1F1F]' : 'text-slate-200'}`}>{previewDoc.title}</div>
                <div className={`text-[11px] mt-0.5 ${isLight ? 'text-[#634B3F]' : 'text-slate-400'}`}>
            ...
            <div className={`space-y-1.5 text-xs p-3.5 rounded-2xl border ${
              isLight
                ? 'bg-[#F3E3D3]/60 border-[#DEC8B2] text-[#3D2C24]'
            ...
            <div className="flex gap-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className={`flex-1 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 text-center transition-all cursor-pointer ${
                  isLight
                    ? 'bg-gradient-to-r from-[#FF8A3D] via-[#E05318] to-[#C2410C] hover:from-[#E05318] hover:to-[#B83808] text-white border-t border-white/30 border-b-[2.5px] border-b-[#9A3412] shadow-md shadow-orange-500/20 active:translate-y-0.5'
                    : 'bg-gradient-to-r from-[#168BFF] to-[#0A56C2] hover:from-[#16C7F2] hover:to-[#168BFF] text-white shadow-md active:scale-95'
                }`}
              >
              ...
              {canEditDocs && (
                <button
                  ...
                  className={`px-3 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${
                    isLight
                      ? 'bg-[#F3E3D3] hover:bg-[#EAD6C4] border border-[#DEC8B2] border-t-white/90 border-b-[2.5px] border-b-[#DEC8B2] text-[#634B3F] active:translate-y-0.5'
                      : 'bg-slate-800 hover:bg-amber-500/20 border border-slate-700 text-slate-300 hover:text-amber-400'
                  }`}
                >
              ...
              {canDeleteDocs && (
                <button
                  ...
                  className={`px-3 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${
                    isLight
                      ? 'bg-[#F3E3D3] hover:bg-rose-100 border border-[#DEC8B2] border-t-white/90 border-b-[2.5px] border-b-[#DEC8B2] text-rose-600 active:translate-y-0.5'
                      : 'bg-slate-800 hover:bg-rose-500/20 border border-slate-700 text-slate-300 hover:text-rose-400'
                  }`}
                >
      ...
      {editingDoc && createPortal(
        <div className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 ${isLight ? 'bg-black/50' : 'bg-black/85'} backdrop-blur-md animate-fade-in select-none`}>
          <div className={`w-full max-w-md rounded-3xl p-5 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto ${
            isLight
              ? 'bg-[#FFF8F1] border-2 border-[#DEC8B2] text-[#1F1F1F]'
              : 'bg-[#07132B] border border-[#168BFF]/30 text-slate-100'
          }`}>
            <div className={`flex items-center justify-between pb-3 border-b ${isLight ? 'border-[#DEC8B2]' : 'border-slate-800'}`}>
              <h3 className={`text-base font-bold ${isLight ? 'text-[#1F1F1F]' : 'text-white'}`}>Edit Document Details</h3>
              <button
                onClick={() => setEditingDoc(null)}
                className={`text-xs cursor-pointer transition-colors ${isLight ? 'text-[#634B3F] hover:text-[#1F1F1F]' : 'text-slate-400 hover:text-white'}`}
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleUpdateDoc} className="space-y-3.5">
              <div>
                <label className={`text-xs font-semibold ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>Document Title *</label>
                <input
                  type="text"
                  required
                  value={editingDoc.title}
                  onChange={(e) => setEditingDoc({ ...editingDoc, title: e.target.value })}
                  className={`w-full mt-1 px-3 py-2 rounded-xl text-xs outline-none transition-all ${
                    isLight
                      ? 'bg-[#F8EFE6] border border-[#DEC8B2] text-[#1F1F1F] focus:border-[#E05318]'
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <CustomSelect
                    ...
                    className={isLight ? '!bg-[#F8EFE6] !border-[#DEC8B2]' : ''}
                  />
                </div>
                <div>
                  <CustomSelect
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={`text-xs font-semibold ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>Document Number</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className={`text-xs font-semibold ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>Issuer Authority</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <CustomDatePicker
                    ...
                    className={`mt-1 ${isLight ? '!bg-[#F8EFE6] !border-[#DEC8B2]' : '!bg-slate-800 !border-slate-700'}`}
                  />
                </div>
                <div>
                  <CustomDatePicker
              ...
              <div>
                <label className={`text-xs font-semibold ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>Tags / Keywords</label>
                <input
                  type="text"
                  ...
                  className={`w-full mt-1 px-3 py-2 rounded-xl text-xs outline-none transition-all ${
                    isLight
                      ? 'bg-[#F8EFE6] border border-[#DEC8B2] text-[#1F1F1F] placeholder-[#947D70] focus:border-[#E05318]'
                      : 'bg-slate-800 border border-slate-700 text-white placeholder-slate-400 focus:border-amber-400'
                  }`}
                />
              </div>

              <div>
                <label className={`text-xs font-semibold ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>Private Notes</label>
                <input
                  type="text"
              ...
              <div className={`flex gap-2 pt-2 border-t ${isLight ? 'border-[#DEC8B2]' : 'border-slate-800'}`}>
                ...
                <button
                  ...
                  className={`flex-1 py-2.5 font-bold rounded-xl text-xs transition-all cursor-pointer ${
                    isLight
                      ? 'bg-gradient-to-r from-[#FF8A3D] via-[#E05318] to-[#C2410C] hover:from-[#E05318] hover:to-[#B83808] text-white border-t border-white/30 border-b-[2.5px] border-b-[#9A3412] shadow-md shadow-orange-500/20 active:translate-y-0.5'
                      : 'bg-gradient-to-r from-amber-500 to-indigo-600 hover:from-amber-400 hover:to-indigo-500 text-white shadow-lg'
                  }`}
                >
                  Save Changes
                </button>
      ...
      {showUploadModal && createPortal(
        <div className={`fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4 ${isLight ? 'bg-black/50' : 'bg-black/90'} backdrop-blur-md overflow-y-auto animate-fade-in`}>
          <div className={`w-full max-w-md rounded-3xl p-4 sm:p-5 shadow-2xl space-y-3.5 flex flex-col max-h-[92vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${
            isLight
              ? 'bg-[#FFF8F1] border-2 border-[#DEC8B2] text-[#1F1F1F]'
              : 'bg-[#07132B] border border-[#168BFF]/30 text-slate-100'
          }`}>
            ...
            {/* Header */}
            <div className={`flex items-center justify-between pb-2 border-b shrink-0 ${isLight ? 'border-[#DEC8B2]' : 'border-slate-800/80'}`}>
              <div className="flex items-center gap-2.5">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${
                  isLight
                    ? 'bg-gradient-to-br from-[#FF8A3D] to-[#E05318] text-white shadow-md shadow-orange-500/25 border-t border-white/40'
                    : 'bg-gradient-to-br from-[#168BFF] to-[#0D59B5] text-white shadow-lg shadow-[#168BFF]/30'
                }`}>
                  <FileText className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h3 className={`text-base font-bold tracking-tight leading-tight ${isLight ? 'text-[#1F1F1F]' : 'text-white'}`}>Add Document</h3>
                  <p className={`text-[11px] font-medium ${isLight ? 'text-[#634B3F]' : 'text-slate-400'}`}>Securely store family documents</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className={`px-2.5 py-1 rounded-full text-[10px] font-semibold flex items-center gap-1 shrink-0 ${
                  isLight
                ...
                <button
                  ...
                  className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors shrink-0 cursor-pointer ${
                    isLight
                      ? 'bg-[#F3E3D3] hover:bg-[#EAD6C4] text-[#634B3F] hover:text-[#1F1F1F] border border-[#DEC8B2]'
            ...
            <form onSubmit={handleSaveDocument} className="space-y-3.5 flex-1 flex flex-col justify-between">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  <label className={`text-xs font-semibold ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>Select Document Type</label>
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-2xl border transition-all cursor-pointer ${
                            isSelected
                              ? isLight
                                ? 'bg-gradient-to-br from-[#FF8A3D] to-[#E05318] text-white border-2 border-[#D3542F] shadow-[0_3px_0_#B8401B] font-bold ring-2 ring-[#E05318]/25'
                                : 'bg-[#168BFF]/20 border-[#16C7F2] text-white shadow-lg shadow-[#168BFF]/20 ring-1 ring-[#16C7F2]/50'
                              : isLight
                              ? 'bg-[#F3E3D3] hover:bg-[#EAD6C4] border-[#DEC8B2] border-t-white/90 border-b-[2.5px] border-b-[#DEC8B2] text-[#634B3F] shadow-sm active:translate-y-0.5'
                ...
                {!uploadedFile ? (
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`p-2.5 rounded-xl text-center text-xs flex items-center justify-center gap-1.5 font-bold transition-all cursor-pointer ${
                        isLight
                          ? 'bg-[#F3E3D3] hover:bg-[#EAD6C4] border border-[#DEC8B2] border-t-white/90 border-b-[2.5px] border-b-[#DEC8B2] text-[#1F1F1F] shadow-sm active:translate-y-0.5'
                          : 'bg-[#030E22]/80 hover:bg-[#168BFF]/10 border border-[#168BFF]/25 text-slate-300'
                      }`}
                    >
                      <Upload className={`w-3.5 h-3.5 ${isLight ? 'text-[#E05318]' : 'text-[#16C7F2]'}`} />
                    ...
                    >
                      <Camera className={`w-3.5 h-3.5 ${isLight ? 'text-[#E05318]' : 'text-cyan-400'}`} />
                  ...
                  <div className={`p-2.5 rounded-xl border flex items-center justify-between ${
                    isLight
                      ? 'bg-[#F3E3D3] border-emerald-600/40 text-[#1F1F1F]'
                      : 'bg-[#030E22]/90 border-emerald-500/40 text-white'
                  }`}>
                    <div className="flex items-center gap-2.5 overflow-hidden">
                      {uploadedFile.type === 'IMAGE' ? (
                        <img
                          ...
                          alt="Preview"
                          className="w-9 h-9 object-cover rounded-lg shrink-0 border border-[#DEC8B2]"
                        />
                      ) : (
                        <div className="w-9 h-9 rounded-lg bg-rose-500/20 border border-rose-500/30 text-rose-500 flex items-center justify-center shrink-0 font-bold text-[11px]">
                          PDF
                        </div>
                      )}
                      <div className="overflow-hidden">
                        <div className={`text-xs font-bold truncate ${isLight ? 'text-[#1F1F1F]' : 'text-white'}`}>{uploadedFile.name}</div>
                        <div className={`text-[10px] font-medium flex items-center gap-1 ${isLight ? 'text-emerald-700' : 'text-emerald-400'}`}>
                    ...
                    <button
                      ...
                      className={`p-1 rounded-lg transition-colors shrink-0 ml-2 cursor-pointer ${
                        isLight
                          ? 'bg-[#EAD6C4] hover:bg-rose-100 text-[#634B3F] hover:text-rose-600'
                ...
                {showOCRResult && (
                  <div className={`p-2.5 rounded-xl text-xs flex items-start gap-2 animate-fadeIn border ${
                    isLight
                      ? 'bg-orange-50 border-orange-200 text-[#634B3F]'
                      : 'bg-[#168BFF]/15 border-[#168BFF]/30 text-cyan-300'
                  }`}>
                    <Sparkles className={`w-4 h-4 shrink-0 mt-0.5 ${isLight ? 'text-[#E05318]' : 'text-[#16C7F2]'}`} />
                    <span>
                      AI extracted <strong className={isLight ? 'text-[#1F1F1F]' : 'text-white'}>{showOCRResult.documentType}</strong> ({showOCRResult.documentNumber}). Expiry: {showOCRResult.expiryDate || 'N/A'}.
                    </span>
                  </div>
                )}
                ...
                <div>
                  <label className={`flex items-center gap-1.5 text-xs font-semibold mb-1 ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>
                    <FileText className={`w-3.5 h-3.5 shrink-0 ${isLight ? 'text-[#E05318]' : 'text-[#16C7F2]'}`} />
                    <span>Document Title <span className={isLight ? 'text-[#E05318]' : 'text-cyan-400'}>*</span></span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className={`w-full px-3.5 py-2.5 rounded-xl text-xs outline-none transition-all ${
                      isLight
                        ? 'bg-[#F8EFE6] border border-[#DEC8B2] text-[#1F1F1F] placeholder-[#947D70] focus:border-[#E05318]'
                        : 'bg-[#030E22]/90 border border-[#168BFF]/30 text-white placeholder-slate-500 focus:border-[#16C7F2]'
                    }`}
                  />
                </div>
                ...
                <div>
                  <CustomSelect
                    ...
                    className={isLight ? '!bg-[#F8EFE6] !border-[#DEC8B2]' : '!bg-[#030E22]/90 !border-[#168BFF]/30'}
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <div>
                    {familyMembers && familyMembers.length > 0 ? (
                      ...
                      <div>
                        <label className={`flex items-center gap-1.5 text-xs font-semibold mb-1 ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>
                          <User className={`w-3.5 h-3.5 shrink-0 ${isLight ? 'text-[#E05318]' : 'text-[#16C7F2]'}`} />
                        ...
                        <input
                          ...
                          className={`w-full px-3.5 py-2.5 rounded-xl text-xs outline-none ${
                            isLight
                              ? 'bg-[#F8EFE6] border border-[#DEC8B2] text-[#1F1F1F] focus:border-[#E05318]'
                              : 'bg-[#030E22]/90 border border-[#168BFF]/30 text-white focus:border-[#16C7F2]'
                          }`}
                        />
                      </div>
                    )}
                ...
                <div>
                  <label className={`flex items-center gap-1.5 text-xs font-semibold mb-1 ${isLight ? 'text-[#634B3F]' : 'text-slate-300'}`}>
                    <Hash className={`w-3.5 h-3.5 shrink-0 ${isLight ? 'text-[#E05318]' : 'text-[#16C7F2]'}`} />
                ...
                <div className={`p-3 rounded-2xl flex items-center justify-between border ${
                  isLight
                    ? 'bg-[#F3E3D3]/70 border-[#DEC8B2]'
                    : 'bg-[#030E22]/90 border-[#168BFF]/30'
                }`}>
                  <div className="flex items-center gap-2.5">
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border ${
                      isLight
                        ? 'bg-orange-100 border-orange-200 text-[#E05318]'
                        : 'bg-[#168BFF]/20 border-[#168BFF]/30 text-[#16C7F2]'
                    }`}>
                      <Bell className="w-4 h-4" />
                    </div>
                    <div>
                      <div className={`text-xs font-semibold ${isLight ? 'text-[#1F1F1F]' : 'text-white'}`}>Set Renewal Reminder</div>
                      <div className={`text-[10px] ${isLight ? 'text-[#634B3F]' : 'text-slate-400'}`}>Auto-reminder 30 days before expiry</div>
                    </div>
                  </div>
                  <button
                    type="button"
                    ...
                    className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      newDoc.auto_create_reminder
                        ? isLight ? 'bg-[#E05318]' : 'bg-[#168BFF]'
                        : isLight ? 'bg-[#DEC8B2]' : 'bg-slate-700'
                    }`}
                  >
                    <span
